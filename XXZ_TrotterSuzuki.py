...
def test_compiling_XXZ(t = 0.1, nqubits = 4, steps = 13):
    h = hamiltonians.XXZ(nqubits=nqubits, delta=0.5)
    return  np.linalg.norm(h.exp(t) - vw_xxz_compiling_phase(nqubits, steps) * (nqubit_XXZ_decomposition(
            nqubits=nqubits,t=0.1,delta=0.5,steps=steps)).unitary())